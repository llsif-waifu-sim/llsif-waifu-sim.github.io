...
text_file = open("../records/id-list-aquors.txt", "w")
text_file.write('[\n')
print '['
for x in range (760,960):
    ...
    if img_url != None and aquors_idol2path(name) != 'none':
        # [(id),(name)]
        text_to_save =  "['" + x_str + "','" + aquors_idol2path(name) +"','no'],\n"
        text_to_prnt =  "['" + x_str + "','" + aquors_idol2path(name) +"','no'],"
        text_file.write(text_to_save)

        print text_to_prnt
        
    if img_url_idol != None and aquors_idol2path(name) != 'none':
        text_to_save =  "['" + x_str + "','" + aquors_idol2path(name) +"','yes'],\n"
        text_to_prnt =  "['" + x_str + "','" + aquors_idol2path(name) +"','yes'],"
        text_file.write(text_to_save)
        print text_to_prnt
